...
const fs = require("fs");
const { v4: uuidv4 } = require("uuid"); // ← importar aquí

const storage = multer.diskStorage({
  destination: (req, file, cb) => cb(null, "uploads/"),
  filename: (req, file, cb) => cb(null, uuidv4() + path.extname(file.originalname)) // ← usar aquí
});
...
router.post("/", upload.single("imagen"), async (req, res) => {
  ...
  try {
    const id = uuidv4(); // puedes reutilizar un id si lo necesitas
